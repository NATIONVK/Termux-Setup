...
def open_github():
    ...
    try:
        # Termux: explicitly hand the URL to Android's browser.
        result = subprocess.run(
            ["am", "start", "-a", "android.intent.action.VIEW", "-d", GITHUB_URL],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL
        )

        if result.returncode != 0:
            # Fallback for Termux environments where `am` is unavailable.
            result = subprocess.run(
        ...
        if result.returncode == 0:
            ...
            # Final fallback to Python's browser handler.
            webbrowser.open(GITHUB_URL)
            print(f"{GREEN}[✓] GitHub launch requested.{RESET}")
...
""")
    ...
    # Second GitHub launch: after the complete setup.
    open_github()
